...
3. `convert`. Mostrar o JPEG + legenda curta. **Parar.**
4. Sem `"publica"` do usuário → não publica.
5. Com `"publica"`: gravar `copy.json` `{formato, caption, alt_texts, source?}` (hashtags já na caption) → `package` → `upload` → `publish`.
